...
- **Gate count: 171** assigned across **10** shards, **7** excluded — re-measured by `make ci-shard-check` on 2026-09-02 (`smoke-runexp`, `smoke-ghostclick`, `smoke-poll`, then `smoke-smep` (DDR-1040, shard 5) took it 167 -> 171). The older text below is kept for its history but the number above is the measured one.
