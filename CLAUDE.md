...
6. **`GLOBAL_FORBIDDEN` is append-only.** Never remove a sentinel. **And never
   ...
   `source <(sed -n '/^GLOBAL_FORBIDDEN=/,/NEXUS KERNEL PANIC.)"$/p' tools/qemu_runner/boot_test.sh)`
   then count the lines — it must be **~71**, not 0. **The terminator in that
