...
| SOVEREIGN FS (SFS) | 🟡 IN PROGRESS | 4 | `kernel/fs/sfs/` (ADR-017/018): inode-based CoW B+tree, 4 KiB blocks. **4d:** format/mount/empty-root. **4e:** CoW B+tree create/lookup/open/readdir — split-on-insert, no in-place mutation, commit via superblock; verified by creating 10 files (forces leaf split + 2-level tree), looking each up, enumerating via in-order tree walk. Next: extents (4f), journal (4g), snapshots (4h), LZ4 (4i). `CAP_FS_SFS_*` reserved. |
