...
| **OPEN-10** | `'btree churn FAIL'` during unrelated `-smp 4` gates | **NARROWED 2026-08-21**: `rc=-1` is now provably `-EPERM` — the `cap_ok(cap, CAP_FS_WRITE)` branch in `vfs_create`. All three DDR-884 candidates (`-EEXIST`/`-ENOSPC`/ADR-032 budget) are eliminated by the value, since DDR-888 + DDR-891 made the space disjoint. See build_status. | **open, gates promotion** |
