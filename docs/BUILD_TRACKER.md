...
| 6.10 | **ACC session rotation** (DDR-815) | ⏳ | NSI 81 `SYS_ACC_ROTATE`, CAP_SOVEREIGN. Per-agent channel table keyed by the agent's Ed25519 verify key (authenticated, unlike a recyclable pid); rotation REVOKES by raising the replay floor past any issuable sequence. `smoke-acc-rotate` 4 arms, **20/20 locally**, unexcluded, shard 2. Awaiting first CI conclusion. |
