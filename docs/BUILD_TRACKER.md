...
| E.3 | **Spawn-depth cap** (DDR-838) | ⏳ | `SPAWN_DEPTH_MAX = 3`. Keyed on **lineage** (`tcb.agent_depth`), not on `is_agent` — fork does not inherit authority flags, so a cap on `is_agent` would be escaped by a single fork. `SYS_FORK` returns `-EAGAIN` (a ceiling, not a permission error) at depth 3. Scoped to agent lineages, so ordinary process trees are untouched — verified `smoke-shell` 5/5. `smoke-spawndepth` **20/20 locally**, shard 3. Awaiting first CI conclusion. |
