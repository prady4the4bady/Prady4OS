= DDR-893 — call-rate data REFUTES the preemption-suppression theory (corrects DDR-890)

**Status:** ACCEPTED. **Corrects a conclusion I recorded in DDR-890.**
**Date:** 2026-08-15
**Lineage:** DDR-887 → DDR-890 (spin count) → DDR-892 (bound) → **DDR-893 (this)**.

## What DDR-890 concluded, and why it was wrong

DDR-890 measured `switch_wait_offcpu_sched` at 2–3 million spins per 500-tick
...
long wait from very many short ones, and DDR-890 had no call count. It should
...
Counters added for calls (wait entered) and bails (DDR-892 bound hit), drained
...
1. **Mean spin per call ≈ 19 iterations** (1 901 883 / 100 014). The individual
   ...
   instructions" describes. **The invariant is not violated. DDR-890's claim that
   it was is withdrawn.**
...
3. **`bails` = 14 and 21 against ~137 000 calls.** DDR-892's 4096 bound is
...
That cannot make a gate overrun by seconds. The mechanism proposed in DDR-890 —
suppression accumulating into multi-second latency — is quantitatively
impossible at this call profile.

**Therefore the post-DDR-887 CI pattern is reading (A):** pre-existing flakes
that the freeze used to hide. Before DDR-887 a run usually died at the freeze
first, so these gates never got to be the first failure. They must now be
classified individually, per gate, against pre-DDR-887 history — not attributed
to a single cause.
...
1. **Keep DDR-892's bound.** It costs nothing (bails ≈ 35 in 137k calls) and
...
4. **Do not revert DDR-887.** Unchanged: the freeze is gone.
...
For each: did it fail on any tip BEFORE DDR-887? If yes → pre-existing, record
...
This is the honest position: DDR-887 fixed the freeze, and the failures behind
