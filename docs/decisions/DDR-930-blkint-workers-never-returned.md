= DDR-930 — `done=0x0` means the blk-integrity workers never RETURNED, and they were not stuck in I/O

**Status:** ACCEPTED (diagnosis + instrument). **No driver change** — §6.0-D.
**Date:** 2026-08-16
**Lineage:** DDR-918 (probe disambiguation) → **DDR-930 (this)**.
...
DDR-918's disambiguation did its job: `reason=workers-late`, **not**
...
The label `workers-late` (which I wrote in DDR-918) is imprecise for this case
...
(DDR-918), i.e. ~8 s, during which four threads on a 4-CPU system produced
nothing at all.
...
CI 31911253495 shard 3 (`smoke-agent-click`, with DDR-928's widened dump):
...
2. The DDR-924 fallback: when `switch_wait_offcpu_sched` hits its bound the CPU
